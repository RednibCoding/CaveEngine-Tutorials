...
// You can adjust this value in the material inspector
uniform float pixelSize = 1.4;

// Pixelation Function
vec2 pixelateUV(vec2 uv, vec2 texResolution, float pixelSize) {
    return floor(uv * texResolution / pixelSize) * (pixelSize / texResolution);
}
...
void main() {
	...
	// Apply Pixelation to UVs
	vec2 texSize = vec2(textureSize(inAlbedo, 0));
	vec2 pixelatedUV = pixelateUV(fsIn.texCoord, texSize, pixelSize);

	vec2 uv = GetMaterialUV();
	vec4 albedo = texture(inAlbedo, pixelatedUV);
